...
export default function AddWorkForm(props: Props) {
  ...
  return (
    <Formik initialValues={props.init} onSubmit={props.submit}>
      {({
        handleChange,
        handleBlur,
        handleSubmit,
        ...
        <View style={addWorkStyle.view}>
          <Text style={addWorkStyle.title}>{t('addWork')}</Text>
          <TextInput
          ...
          <Picker
            ...
            {data?.data.map((e, i) => {
              return <Picker.Item label={e.name} value={e.id} key={i} />;
            })}
          </Picker>
          <TouchableOpacity
            ...
            <Text style={{marginLeft: 10}}>{t('selectDate')}</Text>
          </TouchableOpacity>
